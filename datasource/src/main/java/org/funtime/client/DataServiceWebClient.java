...
public interface DataServiceWebClient {
    ...
    @RequestMapping(value = "/dataset/{when}", method = RequestMethod.POST, consumes = "application/json")
    @RequestLine("POST /dataset/{when}")
    @Headers("Content-Type: application/json")
    void sendData(@PathVariable("when") @Param("when") Long when, LatLngValueMap latLngValueMap);


    @RequestMapping(value = "/dataset/{when}", method = RequestMethod.GET, produces = "application/json")
